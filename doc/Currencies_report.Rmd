...
**Pedro G, Felipe P, Jan S, Brian S and Jorge C**


Business Decisions
---------------------------------------------------------
Assuming I am a Global Potfolio Manager and have limited currency forwards available to hedge my portfolido, with the decomposition of principal components in currency price changes agaist USD I want to group countries in a way that my global portfolio could be more efficiently hedged with few currencies (around 4 in the example below). 


The Data
--------------------------------------------------------------
We got from Bloomberg currency data for all the major currencies in the world against the USD in the last 50 days (short term)
...
Our Approach
---------------------------------------------------
We did the following 6 step analysis, as suggested by the course:
...
3. Check the correlation matrix to see if Factor Analysis makes sense: apparently it does makes sense as the countries with highest eigenvalues in each factor are relatively different amongst each other
...
We decided to use prices instead of returns 
...
We decided to use the spot prices that are more sensitive to daily changes.

This is the correlation matrix of the `r length(factor_attributes_used)` original variable we used for factor analysis 
...
Apparently the factors are independent and can have jointly a good explanation power.

#### Step 4. Develop a scree plot and decide on the number of factors to be derived

We assumed that 4 currency factors is a reasonable assumption for portfolio hedging
...
The scree plot below was used to visualize the data and help determine the number of relevant factors.
...
Canada, Israel, India and Qatar have very different characteristics. Probably these economies have links with countries around them that allow for very different currency fluctuations compared to the USD.
...
There are seven currencies that have low or negative correlation to the USD, and therefore provide potential hedges. However, we need to match that with our initial intuition as some of these currencies have low correlation just because the dollar is rising and their internal economy is bad.  After all, a hedge would need to stop falling under certain conditions.

The seven currencies are the Iraqi Dinar-IQD, Djiboutian Francs-DJF, Indian Rupees-INR, Japanese Yen-JPY, Qatari Riyal-QAR, Israeli New Shekel- INS and Argentinian Peso.

We can remove IQD and DJF as hedges as quickly due to the high risk profile of the countries.  We can also dismiss the JPY as we know that Abemonomics is purposefully putting downward pressure on Yen.  We can dismiss Argentina as a hedge due to the high risk financial system and easily explained low correlation due to economic contraction.

That leaves us with three interesting hedges.  The QAR and INS are both related geopolitically and it would seem benefit seem are affected in opposite ways by oil prices.  India is a fairly closed economy with a strong growth story due to its new government rather than exporting to credit fattened American consumers.

As such we recommend using the QAR, INS and INR to hedge against our USD positions.
